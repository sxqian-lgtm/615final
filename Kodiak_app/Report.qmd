---
title: "An Exploratory Spatial Analysis of Fish Species and Life-Cycle Status on Kodiak Island, Alaska"
format: html
editor: **Shuxin Qian**
---
```{r}
...
Kodiak Island, Alaska, supports diverse fish populations, whose spatial distribution and life-cycle stages are important for fisheries management. This project investigates the spatial patterns of fish species and their life-cycle status across Kodiak Island using data from the Alaska Department of Fish and Game. A total of 2,078 spatial sampling points were analyzed, representing species such as Coho, Sockeye, Chum, Pink, and King Salmon, as well as Dolly Varden and Sheefish, with life status including parr, resident, and smolt.

The data were cleaned, spatially filtered to Kodiak Island, and reshaped into a tidy format, forming the summarized dataset fish_stat. Geospatial analyses included interactive maps of sampling points, species and cycle distribution, and species-specific life-cycle distribution across lakes and streams. Bar charts and pie charts were used to summarize counts and composition, while a Shiny web application was developed to enable exploration of species- and life-cycle-specific patterns.

Results indicate that fish observations are concentrated along streams, and lakes, with notable variation in species composition and life-cycle status distribution across polygons. This project shows how R, Quarto, and Shiny can be integrated to produce professional reports and interactive tools for ecological data analysis, providing insights for fisheries management.
...
The filtered dataset includes lake and stream features, as well as spatial sampling points that record index id, fish species, life-cycle stage, and location information. In the original point data, multiple fish species and life-cycle status information could be recorded within a single observation. To facilitate analysis, these records were separated so that each row represents a single sampling point, a single fish species, and a single life-cycle status.
...
sample_plot
```
This figure presents the spatial distribution of fish sampling points across Kodiak Island. Sampling locations are distributed along river and stream networks, as well as in lake systems, reflecting the importance of freshwater habitats for multiple fish species. The interactive map allows users to inspect individual sampling locations.
...
These plots show variation in species composition and life-cycle representation among polygons, suggesting spatial heterogeneity across Kodiak Island.
...
```
This map displays the spatial distribution of life-cycle stages for a selected fish species across lakes and stream networks. This interactive map allows users to explore where different life-cycle status exist.
###Page 4: Data Table
```{r}
#| echo: false
#| message: false
...
search_data
```
Displays the fish_stat dataset. Users can search, filter, and sort the table to explore specific species or cycle stages.
...
Interactive visualizations in the Shiny application allow users to explore species- and cycle-specific patterns. The analysis provides useful insight into fish distribution of Kodiak island.
